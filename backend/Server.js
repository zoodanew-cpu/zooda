...
// Database connection
mongoose.connect(
  process.env.MONGODB_URI || "mongodb://192.168.0.104:27017/socialdash",
  {
...
app.post("/api/business", authMiddleware, async (req, res) => {
  try {
    ...
    if (existingBusiness) {
      return res
        .status(400)
        .json({ message: "User already has a business registered" });
    }
...
app.post("/api/posts", authMiddleware, async (req, res) => {
  try {
    const { content, platforms, mediaUrl, mediaType, scheduledFor, tags } =
      req.body;
...
app.post(
  ...
  async (req, res) => {
    try {
      const { name, description, price, stock, category, tags, features } =
        req.body;
...
app.post("/api/promotions", authMiddleware, async (req, res) => {
  try {
    ...
    if (!name || !description || !type || !startDate || !endDate) {
      return res
        .status(400)
        .json({ message: "All required promotion fields must be filled" });
    }
...
app.get("/api/promotions", authMiddleware, async (req, res) => {
  try {
    ...
    const promotions = await Promotion.find({ business: business._id }).sort({
      createdAt: -1,
    });
    ...
    const samplePosts = [
      {
        ...
        business: business._id,
        content:
          "Check out our new summer collection! We're excited to launch these amazing products that will make your summer unforgettable.",
      ...
      },
      {
        ...
        business: business._id,
        content:
          "Behind the scenes at our photoshoot! Our team worked hard to bring you the best quality content and products.",
        platforms: ["instagram"],
        status: "published",
        engagement: {
          likes: 89,
    ...
    const samplePromotions = [
      {
        ...
        description:
          "Get 20% off on all summer collection items. Limited time offer!",
        type: "discount",
