...
const businessSchema = new mongoose.Schema(
  {
    ...
    businessWebsite: {
      ...
      default: null,
      trim: true,
      unique: true, // Ensure no duplicate URLs // Basic URL validation
      validate: {
    ...
    status: {
      type: String, // Added 'inactive' for rejected businesses and 'pending' for explicit review phase
    ...
    rejectionReason: {
      // NEW FIELD: Store reason if admin rejects
      type: String,
      default: null,
    },
    suspensionReason: {
      // NEW FIELD: Store reason if admin suspends
      type: String,
      default: null,
...
const PostSchema = new mongoose.Schema(
  {
    ...
    content: { type: String, required: true }, // Single media upload
    ...
    platforms: [
      { type: String, enum: ["facebook", "twitter", "instagram", "linkedin"] },
    ],
    status: {
      type: String,
    ...
    scheduledFor: { type: Date, default: null }, // Engagement metrics
...
async function updateBusinessEngagementRate(businessId) {
  ...
  const engagementRate = parseFloat(
    ((weightedScore / denominator) * 100).toFixed(2)
  );
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (!isMatch)
      return res.status(400).json({ message: "Invalid credentials" }); // Generate JWT token
...
// Track visits from public/client apps
app.post("/api/visits", async (req, res) => {
  try {
    const { businessId, clientId, path, anonymous } = req.body || {};
    console.log("Visit inbound:", {
      businessId,
      clientId,
      path,
      anonymous,
    ...
    await Visit.create({
      ...
      path: path || null,
      anonymous: clientId ? false : !!anonymous,
  ...
  try {
    ...
    res.json({
      ...
      user: {
        name: client.name,
        email: client.email,
        company: client.company, // other fields if needed
      },
    });
  } catch (err) {
...
app.get("/api/profile/:businessid", async (req, res) => {
  try {
    const { businessid } = req.params; // Find the business by ID
    ...
    } // Get social accounts connected to this business // const socialAccounts = await SocialAccount.find({ business: businessid }); // Assuming SocialAccount model exists elsewhere // Get business statistics
    ...
    res.json({
      ...
      profile: {
        ...
        stats: {
          ...
          totalProducts: productCount, // socialAccounts: socialAccounts.length // Assuming 0 if model is missing
        },
        socialAccounts: [
          // ... socialAccounts data
        ],
        recentActivity: [
          ...recentPosts.map((post) => ({
            type: "post",
            description: `Created a new post: "${post.content.substring(
              0,
              50
            )}..."`,
            time: post.createdAt,
...
//         engagement: `${post.likesList?.length || 0} likes, ${
  ...
  async (req, res) => {
    try {
      ...
      if (existingBusiness) {
        return res
          .status(400)
          .json({ message: "You already have a business registered" });
      }
...
app.get("/api/business", async (req, res) => {
  try {
    ...
    const business = await Business.findOne({ user: userId }).populate(
      "user",
    ...
    try {
      ...
      if (businessDescription !== undefined) {
        ...
        if (
          ...
          return res.status(400).json({
            success: false,
            message:
              "Business description must be between 10 and 500 characters",
          });
        }
      }
      ...
      if (businessWebsite !== undefined) {
        updateData.businessWebsite =
          businessWebsite.trim() !== "" ? businessWebsite.trim() : null;
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      const { content, platforms, scheduledFor, tags, category, caption } =
        req.body;
      ...
      if (!business) {
        return res
          .status(400)
          .json({ message: "No business found for this user" });
      }
...
app.get("/api/post/:businessId", async (req, res) => {
  try {
    ...
    const posts = await Post.find({ business: businessId })
      .skip(skip)
      .limit(limit)
      .exec(); // Fetch client details for each post likes
      ...
      posts.map(async (post) => {
        const likesWithUser = await Promise.all(
          post.likesList.map(async (like) => {
            try {
              const client = await Client.findById(like._id).select(
                "name email"
              );
              return client
                ? { _id: like._id, name: client.name, email: client.email }
                : null;
            } catch {
              return null;
            }
...
app.get("/api/promotions/:businessId", async (req, res) => {
  try {
    ...
    const promotions = await Promotion.find({ business: businessId }).sort({
      createdAt: -1,
    });
...
app.delete("/api/promotions/:id", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to delete promotion" });
  }
});
...
app.get("/api/analytics", authMiddleware, async (req, res) => {
  try {
    ...
    const postsCount = await Post.countDocuments({ business: business._id }); // Get products count
    ...
    const totalRevenue = products.reduce(
      (sum, product) => sum + product.sales.revenue,
      0
    );
...
app.get("/api/businesses/:id", async (req, res) => {
  try {
    ...
    const products = await Product.find({
      ...
      .sort({ createdAt: -1 })
      .select("name description price stock images category"); // Get business reviews
...
app.get("/api/business/all", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch businesses" });
  }
});
...
app.get("/api/dashboard/:businessId", async (req, res) => {
  try {
    ...
    const business = await Business.findById(businessId).select(
      "businessName businessCategory createdAt engagementRate"
    );
    if (!business) {
      return res
        .status(404)
    ...
    const [metricsResult, revenueResult, totalProducts, recentPosts, engagementSeries, productsSeries, overviewSeries] =
      await Promise.all([
        ...
        // E. Engagement series (last 7 days)
        Post.aggregate([
          {
            $match: {
          ...
          },
          {
            $project: {
              ...
              likes: { $size: { $ifNull: ["$likesList", []] } },
              comments: { $size: { $ifNull: ["$commentsList", []] } },
        ...
        // F. Products/Sales series (last 6 months)
        Product.aggregate([
          {
            $match: {
              business: businessObjectId,
        ...
        // G. Overview series (last 4 weeks)
        Post.aggregate([
          {
            $match: {
              business: businessObjectId,
    ...
    // Helpers to ensure chart series have consistent x-axis labels even when empty
    ...
    const buildLast4Weeks = () => {
      const weeks = [];
      for (let i = 3; i >= 0; i--) {
        const d = new Date(now.getTime() - i * 7 * 24 * 60 * 60 * 1000);
        const start = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()));
        const day = start.getUTCDay() || 7; // Sunday=7
        start.setUTCDate(start.getUTCDate() - day + 1); // move to Monday
        weeks.push(start.toISOString().slice(0, 10));
      }
      return weeks;
    };

    const filledEngagement = buildLast7Days().map((day) => {
      const found = engagementSeries.find((d) => d.name === day);
      return found || { name: day, engagement: 0, posts: 0 };
    });
    const filledProducts = buildLast6Months().map((label) => {
      const found = productsSeries.find((d) => d.name === label);
      return found || { name: label, products: 0, sales: 0 };
    });
    const filledOverview = buildLast4Weeks().map((label) => {
      const found = overviewSeries.find((d) => d.name === label);
      return (
        found || { name: label, engagement: 0, products: 0, promotions: 0 }
      );
    });
    ...
    res.json({
      ...
      dashboard: {
        ...
        engagementSeries: filledEngagement,
        productsSeries: filledProducts,
        overviewSeries: filledOverview,
        business: {
          name: business.businessName,
          category: business.businessCategory,
          joinedDate: business.createdAt,
...
app.post("/api/follow/:businessId", async (req, res) => {
  ...
  try {
    ...
    const isFollowing = business.followersList.some((followerId) =>
      followerId.equals(userObjectId)
    );

    if (isFollowing) {
      business.followersList = business.followersList.filter(
        (followerId) => !followerId.equals(userObjectId)
      );
    } else {
    ...
    await business.save(); // Optional: update engagementRate when followers change
...
app.get("/api/post/:postId/like-status/:userId", async (req, res) => {
  try {
    ...
    const isLiked = post.likesList.some(
      (like) =>
...
app.post("/api/post/:postId/comment", async (req, res) => {
  try {
    ...
    const populatedComment = {
      ...newComment,
      userId: await Client.findById(userId).select(
        "firstName lastName email avatar username"
      ),
    };
...
app.get("/api/user/:userId/following", async (req, res) => {
  ...
  try {
    ...
    const user = await Client.findById(userId).select("following").populate({
      path: "following",
      select:
        "businessName businessCategory businessDescription businessWebsite logoUrl verified followers totalPosts totalProducts engagementRate",
      model: "Business",
    });
    ...
    const followingBusinesses = user.following.map((business) => {
      ...
      if (businessObj.logoUrl && !businessObj.logoUrl.startsWith("http")) {
        businessObj.logoUrl = `${
          process.env.API_BASE_URL || "https://api.zooda.in"
        }${businessObj.logoUrl.startsWith("/") ? "" : "/"}${
          businessObj.logoUrl
        }`;
      }
      ...
      if (businessObj.businessName) {
        businessObj.username = businessObj.businessName
          .toLowerCase()
          .replace(/[\s.]/g, "_");
...
app.get("/api/posts/following/:userId", async (req, res) => {
  ...
  try {
    ...
    const businesses = await Business.find({
      ...
      "_id businessName businessCategory businessDescription businessWebsite businessAddress businessPhone logoUrl verified followers totalPosts"
    );
    ...
    const posts = await Post.find({
      ...
      .populate({
        path: "business",
        select:
          "businessName businessCategory businessDescription businessWebsite logoUrl verified followers totalPosts createdAt",
        model: "Business",
      })
    ...
    const processedPosts = posts.map((post) => {
      ...
      if (postObj.business && postObj.business.logoUrl) {
        ...
        if (!logoUrl.startsWith("http")) {
          logoUrl = `${process.env.API_BASE_URL || "https://api.zooda.in"}${
            logoUrl.startsWith("/") ? "" : "/"
          }${logoUrl}`;
        }
      ...
      if (postObj.business && postObj.business.businessName) {
        postObj.business.username = postObj.business.businessName
          .toLowerCase()
          .replace(/[\s.]/g, "_");
...
app.get("/api/posts/unfollowed/:userId", async (req, res) => {
  ...
  try {
    ...
    const unfollowedBusinesses = await Business.find({
      ...
      "_id businessName businessCategory businessDescription businessWebsite logoUrl verified followers totalPosts"
    );
    ...
    const processedPosts = posts.map((post) => {
      ...
      if (!postObj.business || !postObj.business.businessName) {
        const businessId = postObj.business?._id || postObj.business;
        const foundBusiness = unfollowedBusinesses.find(
          (b) => b._id.toString() === businessId?.toString()
        );
...
app.get("/api/companies/:companyId", async (req, res) => {
  ...
  try {
    const company = await Business.findById(companyId).select(
      "businessName businessCategory businessDescription businessWebsite businessAddress businessPhone logoUrl status verified followers totalPosts totalProducts engagementRate createdAt"
    );
    ...
    if (companyObj.logoUrl) {
      ...
      if (!logoUrl.startsWith("http")) {
        logoUrl = `${process.env.API_BASE_URL || "http://localhost:3000"}${
          logoUrl.startsWith("/") ? "" : "/"
        }${logoUrl}`;
      }
    ...
    if (companyObj.businessName) {
      companyObj.username = companyObj.businessName
        .toLowerCase()
        .replace(/[\s.]/g, "_");
...
app.post("/api/promotion/:id/track", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to track promotion" });
  }
});
...
app.get("/api/admin/businesses", async (req, res) => {
  try {
    ...
    if (status === "pending") {
      query = {
        $or: [{ status: "pending" }, { status: "active", verified: false }],
      };
    } else if (status === "approved") {
      query = {
        $or: [{ status: "active", verified: true }, { status: "suspended" }],
      };
    }
...
app.put("/api/admin/businesses/:businessId/approve", async (req, res) => {
  try {
    const business = await Business.findByIdAndUpdate(
      ...
      }, // Set to active and fully verified
      { new: true }
    );
...
app.put("/api/admin/businesses/:businessId/reject", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "Business rejected and marked inactive.",
    });
  } catch (error) {
...
app.put("/api/admin/businesses/:businessId/suspend", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error during business suspension" });
  }
});
...
app.put("/api/admin/businesses/:businessId/activate", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error during business activation" });
  }
});
...
app.get("/api/admin/stats", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error fetching platform statistics" });
  }
});
...
app.get("/api/admin/analytics/businesses", async (req, res) => {
  try {
    ...
    const analyticsData = await Business.aggregate([
      ...
      },
      {
        $project: {
          ...
          revenue: { $sum: "$products.sales.revenue" }, // Mocking simple growth since complex comparison requires historical Analytics data
    ...
    res
      .status(500)
      .json({ message: "Server error fetching business analytics" });
  }
});
...
app.put("/api/admin/businesses/:businessId", async (req, res) => {
  try {
    ...
    if (error.code === 11000) {
      return res
        .status(400)
        .json({ message: "Business name or website already exists" });
    }
...
// Delete a post
app.delete(
  "/api/admin/businesses/:businessId/posts/:postId",
  async (req, res) => {
    try {
...
// Delete a promotion
app.delete(
  "/api/admin/businesses/:businessId/promotions/:promotionId",
  async (req, res) => {
    try {
